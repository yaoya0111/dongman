...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
  ...
  <TR>
	...
	<form action="<%=basePath %>/Admin?method=manager" method="post" name="form1" onSubmit="return checkUser33()">
       <TABLE width="100%" border=0 align="center" cellPadding=3 cellSpacing=1 class=tablewidth>
		...
		%>
		  <TR onClick=OnclikeTable(this) align="center" bgColor=#ffffff>
			...
			<TD id=map><%if(Integer.parseInt(list3.get(0).toString())==1){out.print("删除");}else{%><a href="<%=basePath %>Admin?method=delm&id=<%=list3.get(0).toString()%>">删除</a><%} %></TD>
		  </TR>
		<%}}else{%>	
