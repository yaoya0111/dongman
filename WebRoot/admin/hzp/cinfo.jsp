...
function top2(){
   	form3.action="<%=basePath%><%=dir %>/hzp/cinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%><%=dir %>/hzp/cinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/hzp/cinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/hzp/cinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/hzp/cinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/hzp/cinfo.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
    ...
    String dh=Common.toChineseAndTrim(request.getParameter("dh"));
			...
			int cou = cb.getMessageCount("select count(*) from sale where dh='"+dh+"' and title like '%"+title+"%'");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sale where dh='"+dh+"' and title like '%"+title+"%' order by id desc",10);
	...
	<tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
      ...
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><a href="<%=basePath%>HzpAction?method=deltrave&id=<%=pagelist2.get(0).toString()%>" >删除</a></td>
    </tr>
