...
function top2(){
   	form3.action="<%=basePath%><%=dir %>/prep/prep.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%><%=dir %>/prep/prep.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/prep/prep.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/prep/prep.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/prep/prep.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/prep/prep.jsp?page="+page;
      form3.submit();
...
<%
	...
	if(user==null){
		...
		//List pagelist3=cb.getCom("select * from sh where zt='未发货' and xz='购物' order by id desc",9);
%>
<BODY >
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
    ...
    <%  
			...
			int cou = cb.getMessageCount("select count(*) from sh where zt='未发货' and xz='购物' ");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sh where zt='未发货' and xz='购物' order by id desc",9); //带进一个页数，并返回该页所要显示的信息 
