...
function top2(){
   	form3.action="<%=basePath%><%=dir %>/prep/s1info.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%><%=dir %>/prep/s1info.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/prep/s1info.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/prep/s1info.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/prep/s1info.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/prep/s1info.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
    ...
    String esj=request.getParameter("esj"); String dq=request.getParameter("dq"); dq=Common.toChineseAndTrim(dq); 
    float f=0;
			cb.setEVERYPAGENUM(10);
			int cou = cb.getMessageCount("select count(*) from sh where sj='"+dq+"' and tjsj>='"+ksj+"' and tjsj<='"+esj+"' ");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sh where sj='"+dq+"' and tjsj>='"+ksj+"' and tjsj<='"+esj+"' order by id desc",12); //带进一个页数，并返回该页所要显示的信息 
			...
			List pagelist3=(ArrayList)session.getAttribute("qqq");
				...
				for(int i=0;i<pagelist3.size();i++){
					...
					int gwsl=cb.getCount("select sum(sl) from lb where zt!='未提交' and xz='购物' and sid='"+pagelist2.get(0).toString()+"' and tjsj>='"+ksj+"' and tjsj<='"+esj+"'"); 
