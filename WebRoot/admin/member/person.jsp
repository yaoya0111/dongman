		...
		<%
			...
			List pagelist3=(ArrayList)session.getAttribute("qqq");
				...
				for(int i=0;i<pagelist3.size();i++){
					...
					int sum=0;
	 List jflist=cb.getCom("select sid,sl from lb where member='"+pagelist2.get(1).toString()+"' and zt!='未提交'",2);
	 if(!jflist.isEmpty()){
	 	for(int m=0;m<jflist.size();m++){
	 		List list2=(List)jflist.get(m);
		  ...
		  <TR align="center" bgColor=#ffffff>
			...
			<TD id=map><a href="<%=basePath %>ComServlet?method=delmember&id=<%=pagelist2.get(0).toString()%>">删除</a></TD>
		  </TR>
		<%}%>    	
