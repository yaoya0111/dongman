...
function top2(){
   	form3.action="<%=basePath%>member/prep/tj.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>member/prep/tj.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>member/prep/tj.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>member/prep/tj.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>member/prep/tj.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>member/prep/tj.jsp?page="+page;
      form3.submit();
...
<%
	...
	//	List pagelist3=cb.getCom("select * from sh where member='"+member+"'  and xz='购物'  order by id desc",11);
%>
<BODY >
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
	...
	int cou = cb.getMessageCount("select count(*) from sh where member='"+member+"'  and xz='购物'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sh where member='"+member+"'  and xz='购物'  order by id desc",13);
	...
	<tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
      ...
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><%=pagelist2.get(5).toString() %></td>  
       <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><%=pagelist2.get(6).toString() %></td>  
        <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><%=pagelist2.get(7).toString() %></td>  
         ...
         <%if(pagelist2.get(8).toString().equals("已发货")){ %>
         <a href="<%=basePath %>ComServlet?method=qrsh&id=<%=pagelist2.get(0).toString()%>"><%=pagelist2.get(10).toString()%></a>
         <%}else{} %></td>  
         <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><a href="<%=basePath %>member/prep/info.jsp?ddid=<%=pagelist2.get(1).toString()%>">查看</a></td>
      ...
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;">
      <%if(pagelist2.get(10).toString().equals("已确认")){ %>  
      <a href="<%=basePath %>member/prep/pj.jsp?method=pj&ddid=<%=pagelist2.get(0).toString()%>">评价</a>
      <%} %>
      <a href="<%=basePath %>ComServlet?method=delgw2&id=<%=pagelist2.get(0).toString()%>">删除</a>
      </td> 
    </tr>
