...
function top2(){
   	form3.action="<%=basePath%>member/prep/sinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>member/prep/sinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>member/prep/sinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>member/prep/sinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>member/prep/sinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>member/prep/sinfo.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
    ...
    float f=0;
			...
			int cou = cb.getMessageCount("select count(*) from sh where ddh like '%"+ddh+"%' and member='"+member+"' ");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sh where ddh like '%"+ddh+"%' and member='"+member+"' order by id desc",13); //带进一个页数，并返回该页所要显示的信息 
			...
			List pagelist3=(ArrayList)session.getAttribute("qqq");
				...
				for(int i=0;i<pagelist3.size();i++){
					...
					f+=Float.parseFloat(pagelist2.get(11).toString()); 
			%> 
	<tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
      ...
      <%if(pagelist2.get(10).toString().equals("已完成")){ %>  
      <a href="<%=basePath %>member/prep/pj.jsp?ddid=<%=pagelist2.get(0).toString()%>">评价</a>
      <%} %>
...
<TR>
    	   <TD align="right" vAlign=top >
			 ...
			 <TD >
			 <form action="" method="post" name="form3">总计：<%=f %>	
